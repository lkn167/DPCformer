...
def check_data_quality(genotype_array, phenotype_array):
    print("=== Data Quality Check ===")
    genotype_nan = np.isnan(genotype_array).any()
    phenotype_nan = np.isnan(phenotype_array).any()
    print(f"Genotype contains NaN: {genotype_nan}")
    print(f"Phenotype contains NaN: {phenotype_nan}")
    if phenotype_nan:
        nan_count = np.isnan(phenotype_array).sum()
        print(f"Number of NaN in phenotype: {nan_count}")
    genotype_inf = np.isinf(genotype_array).any()
    phenotype_inf = np.isinf(phenotype_array).any()
    print(f"Genotype contains Inf: {genotype_inf}")
    print(f"Phenotype contains Inf: {phenotype_inf}")
    print(f"Phenotype range: [{phenotype_array.min():.4f}, {phenotype_array.max():.4f}]")
    print(f"Phenotype mean: {phenotype_array.mean():.4f}, std: {phenotype_array.std():.4f}")
    print("==========================")
...
def preprocess_data(output_file_path, map_file_path):
    ...
    for i, chunk in enumerate(merged_df_chunks):
        ...
        if chunk['genotype'].isna().any() or chunk['phenotype'].isna().any():
            print(f"Warning: Chunk {i + 1} contains NaN values. Removing rows with NaN...")
            chunk = chunk.dropna(subset=['genotype', 'phenotype'])
        if len(chunk) == 0:
    ...
    print(f"Checking for NaN values in phenotypes array...")
    nan_mask = np.isnan(phenotypes_array)
    if np.any(nan_mask):
        print(f"Found {np.sum(nan_mask)} NaN values in phenotypes. Removing corresponding samples...")
        phenotypes_array = phenotypes_array[~nan_mask]
        one_hot_genotype_array = one_hot_genotype_array[~nan_mask]
    print(f"Final data shapes - Genotype: {one_hot_genotype_array.shape}, Phenotype: {phenotypes_array.shape}")
...
def create_chromosome_groups(snp_count_per_chromosome):
    chromosome_groups = []
    sorted_chromosomes = []
    for ch in snp_count_per_chromosome.keys():
        try:
            if isinstance(ch, int):
                sorted_chromosomes.append(ch)
            elif str(ch).isdigit():
                sorted_chromosomes.append(int(ch))
        except ValueError:
            continue
    sorted_chromosomes = sorted(set(sorted_chromosomes))
    print(f"Available numeric chromosomes: {sorted_chromosomes}")
    for i in range(1, 14):
        a_chr = i
        d_chr = i + 13
        group = []
        if a_chr in sorted_chromosomes:
            group.append(a_chr)
        if d_chr in sorted_chromosomes:
            group.append(d_chr)
        if group:
            chromosome_groups.append(tuple(group))
    print(f"Created {len(chromosome_groups)} chromosome groups:")
    for i, group in enumerate(chromosome_groups):
        print(f"  Group {i + 1}: {group}")
    if len(chromosome_groups) != 13:
        print(f"WARNING: Expected 13 chromosome groups, but got {len(chromosome_groups)} groups!")
        print("This may cause model architecture mismatch with your SHAP analysis code.")
        if len(chromosome_groups) < 13:
            print("Consider checking your chromosome data or adjusting the pairing logic.")
    else:
        print("SUCCESS: Created exactly 13 chromosome groups for cotton tetraploid genome.")
    return chromosome_groups

def prepare_grouped_data(one_hot_genotype_array, phenotypes_array, chromosome_groups, snp_count_per_chromosome, selected_snp_count):
    print("Preparing grouped data with MIC selection...")
    if np.any(np.isnan(phenotypes_array)):
        print("Warning: phenotypes_array still contains NaN values after preprocessing. Removing them...")
        valid_indices = ~np.isnan(phenotypes_array)
        phenotypes_array = phenotypes_array[valid_indices]
        one_hot_genotype_array = one_hot_genotype_array[valid_indices]
        print(f"Data shapes after NaN removal - Genotype: {one_hot_genotype_array.shape}, Phenotype: {phenotypes_array.shape}")
    ...
    for group in chromosome_groups:
        ...
        if np.any(np.isnan(phenotypes_array)):
            print(f"Error: phenotypes_array contains NaN values before MIC selection for group {group}")
            continue
        try:
            ...
            print(f"Error during MIC selection for group {group}: {e}")
            print("Using random SNP selection as fallback...")
            n_available_snps = min(selected_snp_count, n_snps)
            selected_snp_indices = np.random.choice(n_snps, n_available_snps, replace=False)
    ...
    if not grouped_chromosome_data_list:
        print("Warning: No chromosome groups were processed successfully!")
        return None
...
if __name__ == '__main__':
    ...
    output_file_path = '/cotton.csv'
    map_file_path = '/cotton.map'
    ...
    k_folds = 10
    test_ratio = 0.1
    one_hot_genotype_array, phenotypes_array, snp_count_per_chromosome, scaler = preprocess_data(output_file_path, map_file_path)
    check_data_quality(one_hot_genotype_array, phenotypes_array)
    chromosome_groups = create_chromosome_groups(snp_count_per_chromosome)
    print(f"Created chromosome groups: {chromosome_groups}")
    grouped_chromosome_tensors = prepare_grouped_data(one_hot_genotype_array, phenotypes_array, chromosome_groups, snp_count_per_chromosome, selected_snp_count)
    if grouped_chromosome_tensors is None:
        print("Error: Failed to prepare grouped chromosome tensors. Exiting...")
        exit()
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_val_dataset)):
        ...
        for epoch in range(num_epochs):
            ...
            print(f"Epoch {epoch + 1}/{num_epochs} | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f} | Val Pearson: {val_pearson:.4f}")
            with open(log_file, "a") as f:
                f.write(f"{epoch + 1},{avg_train_loss:.4f},{avg_val_loss:.4f},{val_pearson:.4f},{val_spearman:.4f}\n")
            if val_pearson > best_val_pearson:
                best_val_pearson = val_pearson
                torch.save(model.state_dict(), os.path.join(output_dir, 'saved_models', f'best_model_fold_{fold + 1}.pth'))
        ...
        print(f"Fold {fold + 1} Test Results: Loss={avg_test_loss:.4f}, Pearson={test_pearson:.4f}, Spearman={test_spearman:.4f}")
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(f"Average Test Spearman Correlation: {np.mean(cv_test_spearman):.4f} ± {np.std(cv_test_spearman):.4f}\n\n")
